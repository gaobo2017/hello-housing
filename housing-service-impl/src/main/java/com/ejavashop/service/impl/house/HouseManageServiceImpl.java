...
public class HouseManageServiceImpl implements IHouseManageService {
    ...
    public ServiceResult<List<HousingResources>> getHousingResourcesList(Map<String, String> queryMap,
        ...
        try {
            ...
            List<HousingResources> list = houseManageModel.getHousingResourcesList(queryMap, start, size);
            serviceResult.setResult(list);
        } catch (BusinessException e) {
            serviceResult.setMessage(e.getMessage());
            serviceResult.setSuccess(Boolean.FALSE);
            log.error("[HouseManageService][getHousingResourcesList]根据条件分页查询房源信息时出现异常：" + e.getMessage());
        } catch (Exception e) {
            serviceResult.setError(ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR,
                ConstantsEJS.SERVICE_RESULT_EXCEPTION_SYSERROR);
            log.error("[HouseManageService][getHousingResourcesList] param1:" + JSON.toJSONString(queryMap)
